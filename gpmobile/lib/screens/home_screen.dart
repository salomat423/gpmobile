...
class _HomeBody extends StatelessWidget {
      ...
      children: services.map((service) {
        return Builder(
          builder: (context) {
            ...
            VoidCallback? onTap;
            if (name == 'Спорт-бар') {
              onTap = () => Navigator.push(context, MaterialPageRoute(builder: (_) => const SportBarScreen()));
            } else if (name == 'Фитнес-зона') {
              onTap = () => Navigator.push(context, MaterialPageRoute(builder: (_) => const FitnessScreen()));
            } else if (name == 'Восстановление') {
  ...
  Widget _buildScheduleSection(BuildContext context, ThemeData theme, bool isDark) {
      ...
      return Container(
        ...
        child: const Center(
          child: Column(mainAxisSize: MainAxisSize.min, children: [
            ...
            SizedBox(height: 8),
            Text('Ближайших игр нет', style: TextStyle(color: Colors.grey, fontSize: 14)),
          ]),
        ),
      );
    }
    ...
    return Column(
      children: upcoming.map((b) => _buildBookingCard(context, b, isDark, theme)).toList(),
    );
  }

  Widget _buildBookingCard(BuildContext context, Map<String, dynamic> b, bool isDark, ThemeData theme) {
    final court = (b['court_name'] ?? b['court'] ?? 'Корт').toString();
    final status = (b['status'] ?? 'PENDING').toString().toUpperCase();
    final coach = (b['coach_name'] ?? '').toString();
    final rawStart = (b['start_time'] ?? b['date'] ?? '').toString();
    final rawEnd = (b['end_time'] ?? '').toString();

    final dtStart = DateTime.tryParse(rawStart)?.toLocal();
    final dtEnd = DateTime.tryParse(rawEnd)?.toLocal();

    final day = dtStart != null ? '${dtStart.day}' : '--';
    final month = dtStart != null ? _monthsShort[dtStart.month - 1] : '';
    final timeStart = dtStart != null
        ? '${dtStart.hour.toString().padLeft(2, '0')}:${dtStart.minute.toString().padLeft(2, '0')}'
        : '';
    final timeEnd = dtEnd != null
        ? '${dtEnd.hour.toString().padLeft(2, '0')}:${dtEnd.minute.toString().padLeft(2, '0')}'
        : '';
    final timeRange = timeEnd.isNotEmpty ? '$timeStart – $timeEnd' : timeStart;
        ...
        children: [
          ...
          Container(
            ...
            child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
              Text(day, style: TextStyle(fontSize: 22, fontWeight: FontWeight.bold, color: statusColor)),
              Text(month, style: TextStyle(fontSize: 12, color: statusColor.withValues(alpha: 0.8))),
            ]),
          ),
          const SizedBox(width: 12),
          // Info
          Expanded(
            child: Padding(
              ...
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Text(court, style: const TextStyle(fontWeight: FontWeight.w700, fontSize: 15)),
                  const SizedBox(height: 3),
                  Row(children: [
                    Icon(Icons.schedule_rounded, size: 14, color: Colors.grey[500]),
                    const SizedBox(width: 4),
                    Text(timeRange, style: TextStyle(color: Colors.grey[600], fontSize: 13)),
                  ]),
                  if (coach.isNotEmpty) ...[
                    const SizedBox(height: 2),
                    Row(children: [
                      Icon(Icons.sports_rounded, size: 14, color: Colors.grey[500]),
                      const SizedBox(width: 4),
